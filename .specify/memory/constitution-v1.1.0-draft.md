...
> **DRAFT — NOT IN FORCE.** This is the proposed v1.1.0 text, awaiting owner
> approval per `docs/decisions/0003-constitution-v1.1.0.md`. The governing
> document is `constitution.md` (v1.0.0) until this file replaces it. Changes
> against v1.0.0 are marked `[NEW v1.1.0]`; nothing else has been reworded.
...
`[NEW v1.1.0]` **The rule is about the processor, not just the type.** A value
...
`[NEW v1.1.0]` Two consequences that are easy to get wrong once and impossible
to change later:
...
### VI. Serialized state is sectioned and versioned `[NEW v1.1.0]`
...
`[NEW v1.1.0]` Three additions, each covering a case the rules above do not:
...
`[NEW v1.1.0]` **GPU compute is admissible only under all of the following**,
...
`[NEW v1.1.0]` **Perf budgets state bytes touched per tick alongside ms/tick,**
...
## Scope of the access validator `[NEW v1.1.0]`
